...
def load_vectors(dk, va, create_new=False):
    ...
    return AZMAP_NP, ELMAP_NP, GVECTORS

def plot_gradients(dk, va, retinorun):
    # Gradient plot
    spacing =200
    scale = 0.0001 #0.0001
    width = 0.01 #1 #0.01
...
AZMAP_NP, ELMAP_NP, GVECTORS = load_vectors(dk, va, create_new=do_gradients)
if plot:
    fig = plot_gradients(dk, va, retinorun)
    pl.savefig(os.path.join(curr_dst_dir, 'np_gradients.svg'))
    pl.close()

#### Use NEUROPIL to estimate linear model
try:
    REGR_NP = grd.load_models(dk, va, rootdir=rootdi)
...
if do_model:
    ...
    pl.save_fig(os.path.join(curr_dst_dir, '%s.svg' % figname))
    pl.close()
...
if plot:
    ...
    fig = visualize_scatter_on_fov(df_, AZMAP_NP, ELMAP_NP, zimg_r=zimg_r,
                    ...
                    plot_true=plot_true,plot_predicted=predicted, 
                    plot_lines=plot_lines)
    fig.text(0.01, 0.95, 'CTX vs RETINO positions - MEASURED (%s)' % dk)
    pl.savefig(os.path.join(curr_dst_dir, 'fov_true_v_predicted_scatter.svg'))
    pl.close()
...
if plot:
    ...
    pl.savefig(os.path.join(curr_dst_dir, 'deviations.svg'))
    pl.close()
